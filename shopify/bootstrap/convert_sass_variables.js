...
var data = {
	...
	'spacer-1-x':   '{spacer-x}',
	'spacer-1-y':   '{spacer-y}',
	'spacer-2-x':   '({spacer-x} * 1.5)',
	'spacer-2-y':   '({spacer-y} * 1.5)',
	'spacer-3-x':   '({spacer-x} * 3)',
	'spacer-3-y':   '({spacer-y} * 3)',
	'border-width':               '1px',
...
var strBeginsWith = function (str, beginns) {
    return (str.substr(0, beginns.length) == beginns);
}
...
for(key in data) {
	var newDef = {
        ...
        "id": key,
        "label": capitalizeFirstLetter(key.replace(new RegExp('min', "g"), "minimal").replace(new RegExp('max', "g"), "maximal").replace(new RegExp('-xl', "g"), " extra large").replace(new RegExp('-lg', "g"), " large").replace(new RegExp('-md', "g"), " medium").replace(new RegExp('-sm', "g"), " small").replace(new RegExp('-xs', "g"), " extra small").replace(new RegExp('-bg', "g"), " background").replace(new RegExp('-', "g"), " ")),
        "default": data[key],
        "info": ""
	}
	...
	switch(typeof(data[key])) {
		case 'string':
			newDef.type = 'text';
			if(strBeginsWith(data[key], '#') || strBeginsWith(data[key], 'rgb') || strBeginsWith(data[key], 'darken(') || strBeginsWith(data[key], 'lighten(') ) {
				newDef.type = 'color';
			}
		break;
		case 'number':
	...
	settings.push(newDef);
}

settings = JSON.stringify(settings, null, 2);

fs.writeFile('result.json', settings, function(err) {
    if(err) {
      console.log(err);
    } else {
